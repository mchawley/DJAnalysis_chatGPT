...
:root{--bg:#0b1020;--panel:#151d32;--line:#28334f;--muted:#9aa6bf;--text:#edf2ff;--cyan:#54d5ff}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:15px system-ui}.shell{width:min(1600px,96vw);margin:auto;padding:28px 0 60px}.top{display:flex;justify-content:space-between;align-items:end;border-bottom:1px solid var(--line);padding-bottom:22px}.eyebrow{color:var(--cyan);font-size:12px;letter-spacing:.13em;text-transform:uppercase}h1{margin:4px 0;font-size:34px}select,button{background:#1b2740;color:var(--text);border:1px solid #3c4d72;border-radius:8px;padding:10px 12px}select{min-width:340px}button{cursor:pointer;background:var(--cyan);color:#06101b;font-weight:700}.panel{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:20px;margin-top:20px}.hero{display:flex;justify-content:space-between;gap:20px}.title{font-size:22px;font-weight:700}.muted{color:var(--muted)}.stats{display:flex;gap:28px}.stats b{display:block;font-size:20px}.timeline{display:flex;width:100%;height:94px;margin:22px 0 8px;border-radius:8px;overflow:hidden;background:#202a40}.part{min-width:3px;border-right:2px solid #0b1020;padding:9px 8px;color:#0b1020;font-size:11px;font-weight:800;overflow:hidden;cursor:pointer}.part.selected{outline:3px solid white;z-index:1}.INTRO{background:#74b6ff}.GROOVE{background:#71d69b}.BUILD{background:#f6c967}.DROP{background:#ff808d}.BREAKDOWN{background:#ba8df5}.OUTRO{background:#a9b4c8}.CUSTOM{background:#72809a}.legend{display:flex;gap:14px;color:var(--muted);font-size:12px}.dot{display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:5px}.deck{display:grid;grid-template-columns:1.3fr .7fr;gap:20px}.hero-segment{font-size:28px;color:var(--cyan);font-weight:800}.chart{height:155px;width:100%;background:#0f1628;border-radius:10px}.charts{display:grid;grid-template-columns:1fr 1fr;gap:14px}.metric-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:12px}.metric{background:#10182a;border-radius:10px;padding:13px}.metric h3{margin:0 0 10px;font-size:13px}.row{display:grid;grid-template-columns:90px 1fr 38px;gap:8px;align-items:center;font-size:12px;margin:7px 0}.bar{height:8px;background:#28334f;border-radius:99px;overflow:hidden}.fill{height:100%;background:linear-gradient(90deg,#55d5ff,#9d7cff)}.tag{color:var(--muted);font-size:11px}.matches{display:grid;gap:10px;margin-top:12px}.match{display:flex;justify-content:space-between;gap:16px;background:#10182a;border-radius:10px;padding:13px;cursor:pointer}.match:hover{outline:1px solid var(--cyan)}@media(max-width:850px){.deck,.charts{grid-template-columns:1fr}.top,.hero{display:block}.stats{margin-top:15px}select{width:100%}}
...
let selected=0;const $=id=>document.getElementById(id);fetch('/api/tracks').then(r=>r.json()).then(xs=>{xs.forEach(x=>$('track').add(new Option(x.title,x.id)));load()});fetch('/api/summary').then(r=>r.json()).then(x=>$('summary').textContent=`${x.tracks} analyzed tracks · select a section to compare`);$('track').onchange=()=>{selected=0;load()};function meter(label,item,key){let v=item.features[key],p=item.meters[key]||0;return `<div class=row><span>${label}</span><span class=bar><span class=fill style="width:${p}%"></span></span><b>${v?.value??'—'}</b><span></span><span class=tag>${v?.track??'—'} track · ${v?.absolute??'—'} absolute</span></div>`}function chart(values,color){let w=400,h=150,max=Math.max(...values,1),pts=values.map((v,i)=>`${i*w/(values.length-1)},${h-v/max*(h-10)-5}`).join(' ');return `<svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none"><polyline fill="none" stroke="${color}" stroke-width="3" points="${pts}"/></svg>`}function load(i=selected){selected=i;fetch(`/api/track?track_id=${$('track').value}&segment_index=${i}`).then(r=>r.json()).then(x=>{let s=x.segment,t=x.timeline,end=Math.max(...t.map(a=>a.end),1);let line=t.map((a,n)=>`<div onclick="load(${n})" class="part ${a.type} ${n===selected?'selected':''}" style="width:${100*(a.end-a.start)/end}%" title="${a.type}: ${a.start.toFixed(1)}–${a.end.toFixed(1)} seconds">${a.duration>10?a.type:''}<br>${a.start.toFixed(0)}s</div>`).join('');$('deck').innerHTML=`<section class=panel><div class=hero><div><div class=title>${x.title||'Untitled'} <span class=muted>· ${x.artist||'Unknown artist'}</span></div><div class=muted>${x.bpm??'—'} BPM · ${x.key??'—'} · ${t.length} sections</div></div><div class=stats><div><span class=muted>Selected</span><b>${s.type}</b></div><div><span class=muted>Timing</span><b>${s.start.toFixed(1)}–${s.end.toFixed(1)}s</b></div><div><span class=muted>Bars</span><b>${s.bars}</b></div></div></div><div class=timeline>${line}</div><div class=legend><span><i class="dot INTRO"></i>Intro</span><span><i class="dot GROOVE"></i>Groove</span><span><i class="dot BUILD"></i>Build</span><span><i class="dot DROP"></i>Drop</span><span><i class="dot BREAKDOWN"></i>Breakdown</span><span><i class="dot OUTRO"></i>Outro</span></div></section><section class=deck><div><section class=panel><div class=eyebrow>Energy & momentum</div><div class=charts><div><p class=muted>RMS energy</p><div class=chart>${chart(s.charts.rms,'#54d5ff')}</div></div><div><p class=muted>Onset strength</p><div class=chart>${chart(s.charts.onset,'#f6c967')}</div></div></div></section><section class=panel><div class=eyebrow>Segment profile</div><div class=metric-grid><div class=metric><h3>Energy</h3>${meter('Overall',s,'energy')}${meter('Slope',s,'slope')}${meter('Crest',s,'crest')}</div><div class=metric><h3>Bass</h3>${meter('Bass',s,'bass')}${meter('Kick',s,'kick')}${meter('Transient',s,'transient')}</div><div class=metric><h3>Rhythm</h3>${meter('Density',s,'rhythm')}${meter('Groove',s,'groove')}${meter('Syncopation',s,'syncopation')}</div><div class=metric><h3>Spectrum & harmony</h3>${meter('Brightness',s,'brightness')}<div class=row><span>Key</span><b>${s.harmony.key||'—'}</b></div><div class=row><span>Mode</span><b>${s.harmony.mode||'—'}</b></div></div></div></section></div><aside><section class=panel><div class=eyebrow>Similarity decision</div><p>Compare this <b>${s.type}</b> segment against the library.</p><button onclick="similar()">Find similar segments</button><div id=matches class=matches><p class=muted>Matches load on demand.</p></div></section></aside></section>`})}function similar(){let box=$('matches');box.innerHTML='<p class=muted>Comparing normalized fingerprints…</p>';fetch(`/api/similar?track_id=${$('track').value}&segment_index=${selected}`).then(r=>r.json()).then(xs=>box.innerHTML=xs.map(m=>`<div class=match onclick="$('track').value='${m.track_id}';selected=${m.segment_index};load()"><div><b>${m.title}</b><br><span class=muted>${m.segment} · ${m.reasons.join(', ')}</span></div><b>${m.score.toFixed(3)}</b></div>`).join(''))}</script></body></html>"""
...
class InsightsHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if request.path == "/api/track":
            ...
            return self._json({"title": doc.get("metadata", {}).get("title"), "artist": doc.get("metadata", {}).get("artist"), "bpm": doc.get("library", {}).get("bpm"), "key": doc.get("library", {}).get("key"), "segment": {"type": selected.get("segment", "CUSTOM"), "start": selected.get("start_time", 0), "end": selected.get("end_time", 0), "bars": selected.get("bars", 0), "harmony": selected.get("harmonic", {}), "features": features, "meters": {name: self._meter(value["track"], value["absolute"]) for name, value in features.items()}, "charts": {"rms": self._downsample(selected.get("raw_features", {}).get("rms", [])), "onset": self._downsample(selected.get("raw_features", {}).get("onset_strength", []))}}, "timeline": [{"type": item.get("segment", "CUSTOM"), "start": item.get("start_time", 0), "end": item.get("end_time", 0), "duration": item.get("duration", 0)} for item in fingerprints]})
    ...
    def _absolute_label(name, value):
        thresholds = {"energy": (.03, .1), "bass": (.15, .35), "kick": (.08, .25), "rhythm": (.1, .5), "brightness": (1200, 2500), "flatness": (.1, .35), "confidence": (.33, .67)}
        if not isinstance(value, (int, float)) or name not in thresholds: return "—"
        low, high = thresholds[name]
        return "Low" if value < low else "High" if value > high else "Mid"
    @staticmethod
    def _meter(track, absolute):
        return {"Low": 28, "Mid": 58, "High": 88}.get(absolute if absolute != "—" else track, 0)
    @staticmethod
    def _round(value):
