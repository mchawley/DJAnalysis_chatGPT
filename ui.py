...
class InsightsHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if request.path == "/api/track":
            ...
            return self._json({"title": doc.get("metadata", {}).get("title"), "artist": doc.get("metadata", {}).get("artist"), "bpm": doc.get("library", {}).get("bpm"), "key": doc.get("library", {}).get("key"), "fingerprint": selected, "ranges": {name: self._range_label(selected, fingerprints, path) for name, path in paths.items()}, "absolute_ranges": {name: self._absolute_label(name, self._value(selected, path)) for name, path in paths.items()}, "timeline": [{"segment": item.get("segment", "CUSTOM"), "start_time": item.get("start_time", 0), "end_time": item.get("end_time", 0)} for item in fingerprints]})
