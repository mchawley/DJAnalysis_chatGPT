...
function envelope(values,duration){if(!values?.length)return '';let width=800,height=34,max=Math.max(...values.flatMap(value=>[value.energy,value.bass,value.rhythm]),1),line=(key,color)=>values.map((value,index)=>`${(value.start/duration)*width},${height-value[key]/max*(height-5)}`).join(' ')+` ${(values.at(-1).end/duration)*width},${height-values.at(-1)[key]/max*(height-5)}`;return `<svg class="envelope" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" title="Segment-derived full-track envelope: energy, bass, rhythm"><polyline points="${line('energy')}" fill="none" stroke="#54d5ff"/><polyline points="${line('bass')}" fill="none" stroke="#ff7285"/><polyline points="${line('rhythm')}" fill="none" stroke="#f6c967"/></svg>`}
...
class InsightsHandler(BaseHTTPRequestHandler):
    ...
    def _envelope(fingerprints):
        ...
        return [{"start": item.get("start_time", 0), "end": item.get("end_time", 0), "energy": value(item, "energy.overall"), "bass": value(item, "bass.overall"), "rhythm": value(item, "rhythm.density")} for item in fingerprints]
