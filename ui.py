...
function inspect(index=selectedSegment){selectedSegment=index;fetch(`/api/track?track_id=${track.value}&segment_index=${index}`).then(r=>r.json()).then(x=>{let f=x.fingerprint||{},n=(v,k)=>typeof v==='number'?`${v.toFixed(3)} · Track ${x.ranges[k]||'—'} · Absolute ${x.absolute_ranges[k]||'—'}`:'—', end=Math.max(...x.timeline.map(s=>s.end_time),1), line=x.timeline.map((s,i)=>`<div onclick="inspect(${i})" class="part ${s.segment} ${i===selectedSegment?'selected':''}" style="width:${100*(s.end_time-s.start_time)/end}%" title="${s.segment}: ${s.start_time.toFixed(1)}s–${s.end_time.toFixed(1)}s">${s.segment}<br>${s.start_time.toFixed(0)}s</div>`).join(''), box=(title,rows)=>`<section class=feature><h3>${title}</h3>${rows.map(r=>`<div>${r[0]}: <b>${r[1]}</b></div>`).join('')}</section>`; inspector.innerHTML=`<b>${x.title||''}</b> · ${x.artist||''}<div class=timeline>${line}</div><div class=grid><div><span class=label>BPM</span><br>${x.bpm??'—'}</div><div><span class=label>Key</span><br>${x.key??'—'}</div><div><span class=label>Segment</span><br>${f.segment??'—'} · ${f.duration?.toFixed(1)??'—'}s</div></div><div class=features>${box('Energy',[['Overall',n(f.energy?.overall,'energy')],['Start → end',n(f.energy?.start,'energy')+' → '+n(f.energy?.end,'energy')],['Slope',n(f.energy?.slope,'slope')],['Dynamic range',n(f.energy?.crest_factor,'crest')]])}${box('Bass',[['Overall',n(f.bass?.overall,'bass')],['Sub',n(f.bass?.sub,'bass')],['Kick',n(f.bass?.kick,'kick')],['Transient',n(f.bass?.transient_strength,'transient')]])}${box('Rhythm',[['Density',n(f.rhythm?.density,'rhythm')],['Groove',n(f.rhythm?.groove,'groove')],['Syncopation',n(f.rhythm?.syncopation,'syncopation')],['Beat strength',n(f.rhythm?.beat_strength,'rhythm')]])}${box('Harmony',[['Key',f.harmonic?.key??'—'],['Mode',f.harmonic?.mode??'—'],['Confidence',n(f.harmonic?.confidence,'confidence')]])}${box('Spectrum',[['Brightness',n(f.spectrum?.spectral_centroid,'brightness')+' Hz'],['Rolloff',n(f.spectrum?.spectral_rolloff,'brightness')],['Flatness',n(f.spectrum?.spectral_flatness,'flatness')]])}</div>`})}
...
class InsightsHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if request.path == "/api/track":
            ...
            return self._json({"title": doc.get("metadata", {}).get("title"), "artist": doc.get("metadata", {}).get("artist"), "bpm": doc.get("library", {}).get("bpm"), "key": doc.get("library", {}).get("key"), "fingerprint": selected, "ranges": {name: self._range_label(selected, fingerprints, value) for name, value in paths.items()}, "absolute_ranges": {name: self._absolute_label(value, self._value(selected, path)) for name, path in paths.items()}, "timeline": [{"segment": item.get("segment", "CUSTOM"), "start_time": item.get("start_time", 0), "end_time": item.get("end_time", 0)} for item in fingerprints]})
