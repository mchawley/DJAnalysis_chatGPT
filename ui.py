...
let selected=0,currentTrack='',tracks=[],resultIndex=-1;const $=id=>document.getElementById(id);const fmt=v=>{if(typeof v!=='number'||!Number.isFinite(v))return '—';if(v!==0&&Math.abs(v)<.005)return v<0?'−<0.01':'<0.01';return Math.abs(v)>=100?Math.round(v).toString():v.toFixed(2)};const time=v=>typeof v==='number'?v.toFixed(1):'—';const escape=value=>String(value??'').replace(/[&<>'"]/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[char]));const label=item=>item.artist?`${item.title} · ${item.artist}`:item.title;const search=$('track-search'),results=$('track-results');
...
class InsightsHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if request.path == "/api/track":
            ...
            return self._json({"title": doc.get("metadata", {}).get("title"), "artist": doc.get("metadata", {}).get("artist"), "bpm": doc.get("library", {}).get("bpm"), "key": doc.get("library", {}).get("key"), "waveform": self._waveform_view(doc.get("analysis", {})), "segment": {"type": selected.get("segment", "CUSTOM"), "start": selected.get("start_time", 0), "end": selected.get("end_time", 0), "bars": selected.get("bars", 0), "harmony": selected.get("harmonic", {}), "features": features, "meters": {name: self._meter(value["track"], value["absolute"]) for name, value in features.items()}, "charts": {"rms": rms, "onset": onset, "rms_summary": self._rms_summary(rms), "onset_summary": self._onset_summary(onset)}}, "timeline": [{"type": item.get("segment", "CUSTOM"), "start": item.get("start_time", 0), "end": item.get("end_time", 0), "duration": item.get("duration", 0)} for item in fingerprints]})
