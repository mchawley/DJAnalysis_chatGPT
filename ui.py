...
function chooseTrack(item,segment=0){if(!item)return;stopPreview();comparison=null;currentTrack=item.id;selected=segment;search.value=label(item);hideResults();load()}
...
let previewAudio=null,previewActions=null,previewActive=false,previewLoop=false,previewTrackId='';
const previewStyle=document.createElement('style');previewStyle.textContent='.segment-actions.segment-controls{justify-content:space-between;padding:12px;background:#10182a;border-radius:10px}.segment-control-group{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.segment-control-group.preview-group{margin-left:auto}.control-label{color:#9aa6bf;font-size:12px;text-transform:uppercase;letter-spacing:.08em}.preview-status{color:#9aa6bf;font-size:12px;min-width:145px}.icon-button{display:grid;place-items:center;width:38px;height:38px;padding:0;border-radius:50%}.icon-button svg{width:17px;height:17px;fill:currentColor}.icon-button.secondary svg{fill:currentColor}.icon-button.active{background:#f6c967;color:#161000;border-color:#f6c967}.icon-button:focus-visible{outline:2px solid white;outline-offset:2px}@media(max-width:850px){.segment-actions.segment-controls,.segment-control-group.preview-group{justify-content:flex-start;margin-left:0;width:100%}}';document.head.append(previewStyle);
function stopPreview(message='Preview stopped.',clearIntent=true){if(previewAudio){previewAudio.pause();previewAudio.src='';previewAudio=null}if(clearIntent)previewActive=false;let status=document.querySelector('#preview-status');if(status)status.textContent=message}
...
function startPreview(timing){stopPreview('Loading preview…',false);let audio=new Audio(`/api/audio?track_id=${encodeURIComponent(currentTrack)}`);previewAudio=audio;previewTrackId=currentTrack;audio.addEventListener('canplay',()=>{if(audio!==previewAudio||!previewActive)return;audio.currentTime=timing.start;audio.play().then(()=>{$('preview-status').textContent=`Playing ${time(timing.start)}–${time(timing.end)}.`}).catch(()=>{$('preview-status').textContent='This audio format cannot be played by this browser.';previewActive=false})},{once:true});audio.addEventListener('timeupdate',()=>{if(audio.currentTime>=timing.end){if(previewLoop){audio.currentTime=timing.start;audio.play()}else stopPreview('Segment preview complete.')}});audio.addEventListener('error',()=>{if(audio===previewAudio)stopPreview('Original audio is unavailable or unsupported in this browser.')})}
function installPreviewControls(){let actions=document.querySelector('#deck .segment-actions');if(!actions||actions===previewActions)return;let continuePreview=previewActive&&previewTrackId===currentTrack;previewActions=actions;stopPreview('',false);actions.classList.add('segment-controls');let playlistGroup=document.createElement('div');playlistGroup.className='segment-control-group playlist-group';while(actions.firstChild)playlistGroup.append(actions.firstChild);let toggle=$('toggle-segment'),restore=$('restore-track-segments'),including=toggle?.textContent.trim()==='Include';if(toggle){toggle.className='icon-button';toggle.setAttribute('aria-label',including?'Include selected segment':'Exclude selected segment');toggle.title=toggle.getAttribute('aria-label');toggle.innerHTML=`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="${including?'M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6z':'M5 11h14v2H5z'}"/></svg>`}if(restore){restore.className='icon-button secondary';restore.setAttribute('aria-label','Restore all segments');restore.title='Restore all segments';restore.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5V1L7 6l5 5V7a5 5 0 1 1-4.9 6h-2.02A7 7 0 1 0 12 5z"/></svg>'}let previewGroup=document.createElement('div');previewGroup.className='segment-control-group preview-group';previewGroup.innerHTML='<span class="control-label">Segment preview</span><button id="play-preview" class="icon-button" aria-label="Play selected segment" title="Play selected segment"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5v14l11-7z"/></svg></button><button id="stop-preview" class="icon-button secondary" aria-label="Stop preview" title="Stop preview"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 6h12v12H6z"/></svg></button><button id="loop-preview" class="icon-button secondary" aria-label="Loop selected segment" title="Loop selected segment" aria-pressed="false"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 7h9l-2.5-2.5L15 3l5 5-5 5-1.5-1.5L16 9H7a3 3 0 0 0-3 3c0 .7.24 1.34.65 1.85l-1.46 1.46A4.96 4.96 0 0 1 2 12a5 5 0 0 1 5-5zm13 5a5 5 0 0 1-5 5H8l2.5 2.5L9 21l-5-5 5-5 1.5 1.5L8 15h7a3 3 0 0 0 3-3c0-.7-.24-1.34-.65-1.85l1.46-1.46A4.96 4.96 0 0 1 20 12z"/></svg></button><span id="preview-status" class="preview-status">Preview is off.</span>';actions.append(playlistGroup,previewGroup);$('loop-preview').classList.toggle('active',previewLoop);$('loop-preview').setAttribute('aria-pressed',String(previewLoop));$('loop-preview').addEventListener('click',event=>{previewLoop=!previewLoop;event.currentTarget.classList.toggle('active',previewLoop);event.currentTarget.setAttribute('aria-pressed',String(previewLoop))});$('play-preview').addEventListener('click',()=>{let timing=timingFromDeck();if(!timing)return;previewActive=true;startPreview(timing)});$('stop-preview').addEventListener('click',()=>stopPreview());if(continuePreview){let timing=timingFromDeck();if(timing)startPreview(timing)}}
...
function load(index=selected){selected=index;comparison=null;if(!currentTrack)return;fetch(`/api/track?track_id=${encodeURIComponent(currentTrack)}&segment_index=${index}`).then(response=>response.json()).then(data=>{let segment=data.segment,timeline=data.timeline,end=Math.max(...timeline.map(item=>item.end),1);let line=timeline.map((item,itemIndex)=>`<div class="part ${item.type} ${item.included?'':'excluded'} ${itemIndex===selected?'selected':''}" data-segment="${itemIndex}" style="width:${100*(item.end-item.start)/end}%" title="${item.type}: ${time(item.start)}–${time(item.end)}${item.included?'':' · excluded globally'}">${item.duration>10?item.type:''}<br>${time(item.start)}</div>`).join('');$('deck').innerHTML=`<section class="panel"><div class="hero"><div><div class="title">${escape(data.title||'Untitled')} <span class="muted">· ${escape(data.artist||'Unknown artist')}</span></div><div class="muted">${fmt(data.bpm)} BPM · ${escape(data.camelot||data.key||'—')} · ${timeline.length} sections</div></div><div class="stats"><div><span class="muted">Selected</span><b>${segment.type}</b></div><div><span class="muted">Timing</span><b>${time(segment.start)}–${time(segment.end)}</b></div><div><span class="muted">Bars</span><b>${fmt(segment.bars)}</b></div></div></div>${waveform(data.waveform,segment.start,segment.end,end)}${envelope(data.envelope,end)}<div id="timeline" class="timeline">${line}</div><div class="segment-actions"><button id="toggle-segment">${segment.included?'Exclude':'Include'} selected segment</button><button id="restore-track-segments" class="secondary">Restore all segments</button><span class="muted">${segment.included?'Playable in every playlist':'Excluded globally from playlists'}</span></div><div class="beat-summary"><span class="muted">Dominant activity per four-beat bar</span></div>${beatSummary('Onset',segment.patterns.onset)}${beatSummary('Kick',segment.patterns.kick,'kick')}<div class="legend"><span><i class="dot INTRO"></i>Intro</span><span><i class="dot GROOVE"></i>Groove</span><span><i class="dot BUILD"></i>Build</span><span><i class="dot DROP"></i>Drop</span><span><i class="dot BREAKDOWN"></i>Breakdown</span><span><i class="dot OUTRO"></i>Outro</span></div></section><section id="comparison"></section><section class="deck"><div><section class="panel"><div class="eyebrow">Energy & momentum</div><div class="charts">${chartPanel('RMS energy',segment.charts.rms,'#54d5ff','Loudness over time: a rising line builds energy, a flat line stays steady, and dips ease down.',segment.charts.rms_summary)}${chartPanel('Onset strength',segment.charts.onset,'#f6c967','Transient activity over time: frequent peaks suggest dense percussion; isolated peaks are accents or hits.',segment.charts.onset_summary)}</div></section><section class="panel"><div class="eyebrow">Segment profile</div><div class="metric-grid"><div class="metric"><h3>Energy</h3>${meter('Overall',segment,'energy')}${meter('Slope',segment,'slope')}${meter('Crest',segment,'crest')}</div><div class="metric"><h3>Bass</h3>${meter('Bass',segment,'bass')}${meter('Kick',segment,'kick')}${meter('Transient',segment,'transient')}</div><div class="metric"><h3>Rhythm</h3>${meter('Density',segment,'rhythm')}${meter('Groove',segment,'groove')}${meter('Syncopation',segment,'syncopation')}</div><div class="metric"><h3>Spectrum & harmony</h3>${meter('Brightness',segment,'brightness')}<div class="row"><span>Key</span><b>${escape(segment.harmony.camelot||segment.harmony.key||'—')}</b></div><div class="row"><span>Mode</span><b>${escape(segment.harmony.mode||'—')}</b></div></div></div></section></div><aside><section class="panel"><div class="eyebrow">Similarity decision</div><p>Compare this <b>${segment.type}</b> segment against the library.</p><button id="similar">Find similar segments</button><div id="matches" class="matches"><p class="muted">Matches load on demand.</p></div></section></aside></section>`;$('timeline').addEventListener('click',event=>{let part=event.target.closest('.part');if(part)load(Number(part.dataset.segment))});$('toggle-segment').addEventListener('click',()=>setGlobalSegment(segment.index,!segment.included));$('restore-track-segments').addEventListener('click',restoreGlobalSegments);$('similar').addEventListener('click',similar)})}
function setGlobalSegment(segmentIndex,included){fetch('/api/segments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'set_included',track_id:currentTrack,segment_index:segmentIndex,included})}).then(()=>load(segmentIndex))}function restoreGlobalSegments(){fetch('/api/segments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'restore',track_id:currentTrack})}).then(()=>load())}
...
class InsightsHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if request.path == "/api/segments":
            ...
            excluded = SegmentSelectionStore(self.output_root).excluded(track_id)
            return self._json([{"index": i, "type": value.get("segment", "CUSTOM"), "included": i not in excluded} for i, value in enumerate(doc.get("analysis", {}).get("fingerprints", []))])
        if request.path == "/api/track":
            ...
            rms = self._downsample(display_values(selected, "rms"))
            onset = self._downsample(display_values(selected, "onset_strength"))
            return self._json({"title": doc.get("metadata", {}).get("title"), "artist": doc.get("metadata", {}).get("artist"), "bpm": doc.get("library", {}).get("bpm"), "key": doc.get("library", {}).get("key"), "camelot": self._camelot(doc.get("library", {}).get("key")), "waveform": self._waveform_view(doc.get("analysis", {})), "envelope": self._envelope(fingerprints), "segment": {"index": index, "included": index not in excluded, "type": selected.get("segment", "CUSTOM"), "start": selected.get("start_time", 0), "end": selected.get("end_time", 0), "bars": selected.get("bars", 0), "harmony": selected.get("harmonic", {}), "patterns": {"onset": self._pattern_summary(selected.get("rhythm", {}).get("onset_pattern", [])), "kick": self._pattern_summary(selected.get("bass", {}).get("kick_pattern", []))}, "features": features, "meters": {name: self._meter(value["track"], value["absolute"]) for name, value in features.items()}, "charts": {"rms": rms, "onset": onset, "rms_summary": self._rms_summary(rms), "onset_summary": self._onset_summary(onset)}}, "timeline": [{"index": item_index, "included": item_index not in excluded, "type": item.get("segment", "CUSTOM"), "start": item.get("start_time", 0), "end": item.get("end_time", 0), "duration": item.get("duration", 0)} for item_index, item in enumerate(fingerprints)]})
    ...
    def do_POST(self):
        ...
        if request_path == "/api/segments":
            try:
                ...
                return self._json({"error": "Invalid segment request"})
            selections = SegmentSelectionStore(self.output_root)
            if payload.get("action") == "set_included":
                return self._json(selections.set_included(payload.get("track_id", ""), payload.get("segment_index"), bool(payload.get("included"))) or {})
            if payload.get("action") == "restore":
                return self._json(selections.restore(payload.get("track_id", "")))
            return self._json({"error": "Unknown segment action"})
    ...
    def _playlist_detail(self, playlist_id):
        ...
        store = PlaylistStore(self.output_root)
        selections = SegmentSelectionStore(self.output_root)
        tracks = [self._playlist_track(entry["trackId"], entry["id"], selections.excluded(entry["trackId"])) for entry in store.entries(playlist)]
