...
function inspect(index=selectedSegment){selectedSegment=index;fetch(`/api/track?track_id=${track.value}&segment_index=${index}`).then(r=>r.json()).then(x=>{let f=x.fingerprint||{},n=v=>typeof v==='number'?v.toFixed(3):'—', end=Math.max(...x.timeline.map(s=>s.end_time),1), line=x.timeline.map((s,i)=>`<div onclick="inspect(${i})" class="part ${s.segment} ${i===selectedSegment?'selected':''}" style="width:${100*(s.end_time-s.start_time)/end}%" title="${s.segment}: ${s.start_time.toFixed(1)}s–${s.end_time.toFixed(1)}s">${s.segment}<br>${s.start_time.toFixed(0)}s</div>`).join(''), box=(title,rows)=>`<section class=feature><h3>${title}</h3>${rows.map(r=>`<div>${r[0]}: <b>${r[1]}</b></div>`).join('')}</section>`; inspector.innerHTML=`<b>${x.title||''}</b> · ${x.artist||''}<div class=timeline>${line}</div><div class=grid><div><span class=label>BPM</span><br>${x.bpm??'—'}</div><div><span class=label>Key</span><br>${x.key??'—'}</div><div><span class=label>Segment</span><br>${f.segment??'—'} · ${f.duration?.toFixed(1)??'—'}s</div></div><div class=features>${box('Energy',[['Overall',n(f.energy?.overall)],['Start → end',n(f.energy?.start)+' → '+n(f.energy?.end)],['Slope',n(f.energy?.slope)],['Dynamic range',n(f.energy?.crest_factor)]])}${box('Bass',[['Overall',n(f.bass?.overall)],['Sub',n(f.bass?.sub)],['Kick',n(f.bass?.kick)],['Transient',n(f.bass?.transient_strength)]])}${box('Rhythm',[['Density',n(f.rhythm?.density)],['Groove',n(f.rhythm?.groove)],['Syncopation',n(f.rhythm?.syncopation)],['Beat strength',n(f.rhythm?.beat_strength)]])}${box('Harmony',[['Key',f.harmonic?.key??'—'],['Mode',f.harmonic?.mode??'—'],['Confidence',n(f.harmonic?.confidence)]])}${box('Spectrum',[['Brightness',n(f.spectrum?.spectral_centroid)+' Hz'],['Rolloff',n(f.spectrum?.spectral_rolloff)],['Flatness',n(f.spectrum?.spectral_flatness)]])}${box('Vocals',[['Presence',n(f.vocals?.presence)],['Instrumental',n(f.vocals?.instrumental_probability)],['Status','model pending']])}</div>`})}
