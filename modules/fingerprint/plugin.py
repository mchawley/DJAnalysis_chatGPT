...
class FingerprintPlugin:
    ...
    def process(self, document, path):
        import librosa
        analysis = document["analysis"]; audio, sample_rate = librosa.load(path, sr=None, mono=True)
        segments = self._segments(analysis, document.get("library", {}))
        builder = FingerprintBuilder(audio, sample_rate, beat_positions=analysis.get("beatPositions", []))
        document.setdefault("analysis", {})["fingerprints"] = [builder.build(segment).to_dict() for segment in segments]
    ...
    def _segments(self, analysis, library):
        ...
        for index, phrase in enumerate(phrases):
            ...
            segment_type = self.KIND_TYPES.get(phrase.get("kind"), "CUSTOM")
